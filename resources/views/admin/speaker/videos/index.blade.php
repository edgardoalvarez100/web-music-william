		...
		<table class="table table-striped- table-bordered table-hover table-checkable" id="kt_table_1">
			...
			<tbody>
				...
				<tr>
					...
					<td>{{ $video->id }}</td>
					<td>{{ $video->title }}</td>
					<td><img src="{{ url('storage/videos/'.$video->cover) }}" class="img-thumbnail" style='width:100px;'></td>
					
					<td>{{ $video->created_at }}</td>
					<td>{{ $video->updated_at->diffForHumans() }}</td>
					<td nowrap>
						
						<a title="Edit details" class="btn btn-sm btn-clean btn-icon btn-icon-md" href="{{route('video.edit',[$video->serie->id ,$video->id])}}">
							<i class="la la-edit"></i>
						</a>
						<form action="{{ route('video.delete',[$video->serie->id ,$video->id]) }}" method="POST" class="d-inline">
							{{ method_field('DELETE') }}
							{{ csrf_field() }}
