...
<div class="kt-portlet kt-portlet--mobile">
		...
		<div class="kt-portlet__head-toolbar">
			<div class="kt-portlet__head-wrapper">
				<div class="kt-portlet__head-actions">
					...
					&nbsp;
					<a href="{{ route('video.create', $serie->id) }}" class="btn btn-brand btn-elevate btn-icon-sm">
	...
	<div class="kt-portlet__body">
		...
		<table class="table table-striped- table-bordered table-hover table-checkable" id="kt_table_1">
			...
			<tbody>
				@foreach($serie->videos as $vid)
				<tr>
					<td>{{ $vid->id }}</td>
					<td>{{ $vid->title }}</td>
					<td>{{ $vid->week }}</td>
					<td><img src="{{ url('storage/videos/'.$vid->cover) }}" class="img-thumbnail" style='width:100px;'></td>
					<td>@if($vid->speaker)
						{{ $vid->speaker->name }}
					@endif</td>
					<td>{{ $vid->date }}</td>
					<td>{{ $vid->updated_at->diffForHumans() }}</td>
					<td nowrap>						
						<a title="Edit details" class="btn btn-sm btn-clean btn-icon btn-icon-md" href="{{route('video.edit', [$serie->id, $vid->id])}}">
							<i class="la la-edit"></i>
						</a>
						<form action="{{ route('video.delete',[$serie->id,$vid->id]) }}" method="POST" class="d-inline">
							{{ method_field('DELETE') }}
							{{ csrf_field() }}
