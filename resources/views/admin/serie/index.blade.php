		...
		<div class="kt-portlet__head-toolbar">
			<div class="kt-portlet__head-wrapper">
				<div class="kt-portlet__head-actions">
					...
					&nbsp;
					<a href="{{ route('serie.create') }}" class="btn btn-brand btn-elevate btn-icon-sm">
		...
		<table class="table table-striped- table-bordered table-hover table-checkable" id="kt_table_1">
			...
			<tbody>
				...
				<tr>
					...
					<td>{{ $serie->id }}</td>
					<td>{{ $serie->name }}</td>
					<td><img src="{{ url('storage/series/'.$serie->cover) }}" class="img-thumbnail" style='width:100px;'></td>
					
					<td>{{ $serie->date }}</td>
					<td>{{ $serie->videos()->count() }}</td>
					<td>{{ $serie->updated_at->diffForHumans() }}</td>
					<td nowrap>
						<a title="Edit details" class="btn btn-sm btn-clean " href="{{route('video',$serie->id)}}">
							Videos
						</a>						
						<a title="Edit details" class="btn btn-sm btn-clean btn-icon btn-icon-md" href="{{route('serie.edit',$serie->id)}}">
							<i class="la la-edit"></i>
						</a>
						<form action="{{ route('serie.delete',$serie->id) }}" method="POST" class="d-inline">
							{{ method_field('DELETE') }}
							{{ csrf_field() }}
